...
int main() {
	...
	for (int i = 0; i < n_iterations; ++i) {
		multiply_simple(A, B, C1, n);
	}
	gettimeofday(&end, NULL);
	ss << n << " " << (end.tv_sec - start.tv_sec)+1e-6*(end.tv_usec - start.tv_usec);
#ifndef NDEBUG
	...
	gettimeofday(&start, NULL);
	for (int i = 0; i < n_iterations; ++i) {
		multiply_Strassen(n, A, n, B, n, C3, n);
	}
	gettimeofday(&end, NULL);
	ss << n << " " << (end.tv_sec - start.tv_sec)+1e-6*(end.tv_usec - start.tv_usec);
#ifndef NDEBUG
    ...
    for (int i=0; i<n*n; ++i)
        if (A[i]!=0)
            throw std::runtime_error("Results C3 and C1 not same!");


    std::cout << "** RESULTS **" << std::endl;
    std::cout << "Matrix_size  multiply_simple  multiply  multiply_Strassen" << std::endl;
